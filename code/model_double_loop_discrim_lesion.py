...
def simulate_model(
    ...
    alpha_actor_pos_1 = 0.5
    alpha_actor_neg_1 = 0.5

    alpha_actor_pos_2 = 8e-1
    alpha_actor_neg_2 = 8e-1
    ...
    for sim in range(n_simulations):
        ...
        for trl in range(n_trials - 1):
            ...
            # visual input
            xg = np.arange(0, vis_dim, 1)

            vis = np.exp(-((xg - x) ** 2 / (2 * vis_sigma**2)))
            vis = np.reshape(vis, (vis.shape[0], 1))
            vis_rec.append(vis)
...
def inspect_lesion():
    ...
    sns.lineplot(
        data=d[
            ((d["lesion_loc"] == "DMS") | (d["lesion_loc"] == "Control"))
            & ((d["lesion_trial"] == 100) | (d["lesion_trial"] == -1))
        ],
        x="trial",
        y="Accuracy",
        hue="lesion_sd",
    ...
    [x.axvline(100, linestyle="--", color="black") for x in [ax[1, 0], ax[1, 1]]]
    [sns.move_legend(x, "lower right") for x in ax.flat]
    ax[0, 0].set_title("DMS")
    ax[0, 1].set_title("DLS")
    plt.tight_layout()
    plt.savefig("../figures/model_discrim_lesion.png")
    plt.close()

   d.to_csv("../output/model_discrim_lesion.csv", index=False)
