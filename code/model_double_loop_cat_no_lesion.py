...
def make_stim_cats():
    ...
    # NOTE: what does this set up mean?
    ...
    def sample_within_ellipse(mean, n_samples):
        ...
        # NOTE: would like a visual reprsentation of this if possible to help
        # understanding
        ...
        # NOTE: what does np.dot do? rotation_matrix? (that's above, will
        # hopefully have clarity on what that sets up) + what does using a mean
        # for points do?
        # Apply rotation
    ...
    # NOTE: technical question, what is this doint? what do the [1] and [2]
    # mean here for this array?
    # Define the labels
    ...
    # NOTE: need an explanation of this here, why??
    ...
    # NOTE: why shuffle rows? what does it do?
    # shuffle rows of ds
...
def simulate_model(
    ...
    # NOTE: is this defining a range of vis cotex cells? so it's not just one,
    # but an array of cells?
    vis_dim = 100
    vis_sigma = 10
    vis = np.zeros((vis_dim, vis_dim))

    # NOTE: no premotor or m1??
    ...
    # NOTE: haven't seen it worded or coded like this before. why? what's
    # different here?
    ...
    for sim in range(n_simulations):
        ...
        for trl in range(n_trials - 1):
            ...
            # NOTE: what does .meshgrid do? is this creating a meshgrid of
            # visual neurons?
            # visual input
            xg, yg = np.meshgrid(np.arange(0, vis_dim, 1), np.arange(0, vis_dim, 1))

            # NOTE: what does this mean? what is it saying? i'm sure it will be
            # in a paper somewhere
            vis = np.exp(-(((xg - x) ** 2 + (yg - y) ** 2) / (2 * vis_sigma**2)))
            vis_rec.append(vis)

            # NOTE: what does .flatten do? and what is it doing here?
            # stage 1: dms
            dms_A[sim, trl] = np.dot(vis.flatten(), w_vis_dms_A.flatten())
            dms_B[sim, trl] = np.dot(vis.flatten(), w_vis_dms_B.flatten())

            # NOTE: why are we totalling dms_A and dms_B? what purpose is that
            # serving?
            total = dms_A[sim, trl] + dms_B[sim, trl]
            dms_A[sim, trl] /= total
            dms_B[sim, trl] /= total

            # NOTE: similar to model_spiking, but not quite. why no mu and sig? 
            # simulate lesion by adding noise. also, i'm confused, where are
            # the variables like v, u, g etc.? and there's weird equations
            # below...
            if trl > trl_lesion:
            ...
            # NOTE: a) what does this mean? and b) what is this syntax saying?
