...
def simulate_model(
    ...
    alpha_actor_pos_1 = 0.01
    alpha_actor_neg_1 = 0.01

    alpha_actor_pos_2 = 0.01
    alpha_actor_neg_2 = 0.01
...
def inspect_lesion():
    ...
    sns.lineplot(
        data=d[
            ((d["lesion_loc"] == "DMS") | (d["lesion_loc"] == "Control"))
            & ((d["lesion_trial"] == 4000) | (d["lesion_trial"] == -1))
        ],
        x="trial",
        y="Accuracy",
        hue="lesion_sd",
    ...
    [x.axvline(4000, linestyle="--", color="black") for x in [ax[1, 0], ax[1, 1]]]
    [sns.move_legend(x, "lower right") for x in ax.flat]
    ax[0, 0].set_title("DMS")
    ax[0, 1].set_title("DLS")
    plt.tight_layout()
    plt.savefig("../figures/model_discrim_lesion_simple_vis.png")
    plt.close()

    d.to_csv("../output/model_discrim_lesion_simple_vis.csv", index=False)
